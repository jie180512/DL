...
import numpy as np
from test_layers import conv_forward
...
def conv_backward(next_dz, K, z, padding=(0, 0), strides=(1, 1)):
    ...
    dK = conv_forward(swap_z, padding_next_dz, np.zeros((D,), dtype=np.float64))
...
def test_conv():
    ...
    from losses import mean_squared_loss
...
def test_conv_and_max_pooling():
    ...
    next_z = conv_forward(z, K, b)
    y_pred = max_pooling_forward(next_z, pooling=(2, 2))
    ...
    for i in range(10000):
        ...
        next_dz = max_pooling_backward(dy, next_z, pooling=(2, 2))
