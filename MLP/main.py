...
def loadData(which):
    images = np.load("../data/mlp_data/mnist_{}_images.npy".format(which))
    labels = np.load("../data/mlp_data/mnist_{}_labels.npy".format(which))
...
np.random.seed(1)
mlp = MLPClassifier((50,), learning_rate=0.05, max_epoch=100, activation='relu')
